...
function drawRobot() {
  ...
  const shoulderY = baseY - 133 + bob;
  const armWiggle = walking ? Math.sin(performance.now() * 0.014) * 0.22 : 0;
  ...
  drawSpringArm(robot.x - 49, shoulderY, -2.65 + armWiggle, state.carrying && side < 0);
  drawSpringArm(robot.x + 49, shoulderY, -0.49 - armWiggle, state.carrying && side > 0);

  ctx.save();
  ctx.translate(robot.x, baseY + bob);
  roundedRect(-17, -49, 34, 33, 11, "#283756", "#17223e", 3);
  circle(0, -23, 29, "#25263c", "#15203a", 4);
  circle(0, -23, 17, "#4d4a53", "#252735", 3);
  circle(0, -23, 8, "#f25a3b", "#a83335", 2);
  ctx.fillStyle = "#2b3656";
  ctx.beginPath(); ctx.moveTo(-33, -51); ctx.quadraticCurveTo(0, -76, 33, -51); ctx.lineTo(28, -43); ctx.lineTo(-28, -43); ctx.closePath(); ctx.fill();
  ctx.restore();

  // The lower body stays stable while the head rotates independently.
  ctx.save();
  ctx.translate(robot.x, baseY + bob);
  ctx.beginPath(); ctx.moveTo(-52, -143); ctx.lineTo(52, -143); ctx.lineTo(28, -64); ctx.quadraticCurveTo(0, -47, -28, -64); ctx.closePath();
  const body = ctx.createLinearGradient(-40, -145, 35, -55);
  body.addColorStop(0, "#ffd936"); body.addColorStop(1, "#e69b10");
  ctx.fillStyle = body; ctx.fill(); ctx.lineWidth = 4; ctx.strokeStyle = "#34405e"; ctx.stroke();
  roundedRect(-52, -151, 104, 15, 8, "#343b59", "#26304b", 2);
  for (let index = 0; index < 4; index += 1) circle(-18 + index * 12, -124, 4, index === 0 ? "#22b6d5" : "#ffe142", "#b37d13", 1);
  ctx.restore();

  drawRobotHead(robot.x, baseY - 210 + bob, robot.headAngle);
...
function drawSpringArm(x, y, angle, holding) {
  ...
  for (let index = 0; index < 7; index += 1) {
    ctx.strokeStyle = index % 2 ? "#8891aa" : "#f5f5fb";
    ctx.lineWidth = 4;
    ctx.beginPath(); ctx.moveTo(8 + index * 8, -7); ctx.lineTo(13 + index * 8, 7); ctx.stroke();
  }
  ctx.translate(72, 0);
  ctx.strokeStyle = "#26304f";
  ctx.lineWidth = 16;
  ctx.lineCap = "round";
  ctx.beginPath(); ctx.arc(0, 0, 17, holding ? -2.1 : -2.55, holding ? 2.1 : 2.55); ctx.stroke();
  ctx.lineCap = "butt";
  ctx.restore();
}
...
function drawRobotHead(x, y, angle) {
  ...
  circle(0, -81, 13, "#fff37c", "#f4d84d", 2); ctx.restore();
  roundedRect(-6, -69, 12, 16, 4, "#d6d8e3", "#65708b", 2);

  ctx.beginPath(); ctx.moveTo(-66, 37); ctx.quadraticCurveTo(-74, -22, -47, -59); ctx.quadraticCurveTo(0, -88, 47, -59); ctx.quadraticCurveTo(74, -22, 66, 37); ctx.quadraticCurveTo(54, 61, 0, 70); ctx.quadraticCurveTo(-54, 61, -66, 37); ctx.closePath();
  const head = ctx.createLinearGradient(-55, -62, 54, 65);
  head.addColorStop(0, "#ffe23e"); head.addColorStop(0.55, "#f5bf14"); head.addColorStop(1, "#d9990d");
  ctx.fillStyle = head; ctx.fill(); ctx.lineWidth = 4; ctx.strokeStyle = "#35405e"; ctx.stroke();

  ctx.strokeStyle = "#33415f"; ctx.lineWidth = 11; ctx.lineCap = "round";
  ctx.beginPath(); ctx.moveTo(-54, 37); ctx.lineTo(54, 37); ctx.stroke(); ctx.lineCap = "butt";
  ctx.fillStyle = "#33415f"; ctx.fillRect(-44, -39, 22, 8); ctx.fillRect(22, -39, 22, 8);

  if (eyeClosed) {
    ctx.strokeStyle = "#563a8c"; ctx.lineWidth = 5;
    for (const eyeX of [-28, 28]) { ctx.beginPath(); ctx.arc(eyeX, -11, 15, 0.2, Math.PI - 0.2); ctx.stroke(); }
  } else {
    drawEye(-28, -10, gaze, mood === "wrong");
    drawEye(28, -10, gaze, mood === "wrong");
  }
...
function drawEye(x, y, gaze, worried) {
  circle(x, y, 20, "#83bd32", "#4d6940", 3);
  circle(x, y, 15, "#36506d", null);
  circle(x + gaze * 0.28, y - 1, 10, worried ? "#4b325d" : "#1c2b48", null);
  circle(x + gaze * 0.28 - 3, y - 6, 4, "#fff", null);
}
